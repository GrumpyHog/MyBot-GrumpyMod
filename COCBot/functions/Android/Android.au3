		...
		While Not $hMutex And $iPort < $iPortStart + $iPortRange
			...
			If $g_iAndroidAdbMinitouchMode = 0 Then
				...
				If $iTcpMtIdx = -1 Or ($iTcpMtIdx > 0 And $aTcpTable[$iTcpMtIdx][0] = "adb.exe") Then
					; use minitouch port
					$iMtPort = $iPort + 1000
					SetLog("Use TCP Forward for Mini Touch")
				Else
					SetLog("Use ADB for Mini Touch")
				EndIf
			EndIf
...
Func AndroidZoomOut($loopCount = 0, $timeout = Default, $bMinitouch = Default, $wasRunState = Default)
	If $g_bOnBuilderBaseEnemyVillage Then
		SetLog("Running minitouch ZoomOutTOP script", $COLOR_INFO)
...
Func AndroidSCID4Swipe($loopCount = 0, $timeout = Default, $bMinitouch = Default, $wasRunState = Default)
	Return AndroidAdbScript("SCID4Swipe", Default, $timeout, $bMinitouch, $wasRunState)
EndFunc   ;==>AndroidZoomOut

Func AndroidSCID5Swipe($loopCount = 0, $timeout = Default, $bMinitouch = Default, $wasRunState = Default)
	Return AndroidAdbScript("SCID5Swipe", Default, $timeout, $bMinitouch, $wasRunState)
EndFunc   ;==>AndroidZoomOut

Func AndroidSCID6Swipe($loopCount = 0, $timeout = Default, $bMinitouch = Default, $wasRunState = Default)
	Return AndroidAdbScript("SCID6Swipe", Default, $timeout, $bMinitouch, $wasRunState)
EndFunc   ;==>AndroidZoomOut

Func AndroidSCID7Swipe($loopCount = 0, $timeout = Default, $bMinitouch = Default, $wasRunState = Default)
	Return AndroidAdbScript("SCID7Swipe", Default, $timeout, $bMinitouch, $wasRunState)
EndFunc   ;==>AndroidZoomOut
...
Func AndroidClickDrag($x1, $y1, $x2, $y2, $wasRunState = Default, $sleepStart = 250, $sleepMove = 10, $sleepEnd = 1000, $iPixStep = 10)
	...
	Return AndroidMinitouchClickDrag($x1, $y1, $x2, $y2, $wasRunState, $sleepStart, $sleepMove, $sleepEnd, $iPixStep)
EndFunc   ;==>AndroidClickDrag

Func AndroidMinitouchClickDrag($x1, $y1, $x2, $y2, $wasRunState = Default, $sleepStart = 250, $sleepMove = 10, $sleepEnd = 1000, $iPixStep = 10)
	AndroidAdbLaunchShellInstance($wasRunState)
	If $g_iAndroidAdbMinitouchMode = 0 Then
		If $g_bAndroidAdbMinitouchSocket < 1 Then
    ...
    ;SetLog("sleepStart: " & $sleepStart)
	;SetLog("sleepMove: " & $sleepMove)
	;SetLog("sleepEnd: " & $sleepEnd)
	;SetLog("iPixStep: " & $iPixStep)
	...
	Local $steps = Int(($screen_w * $iPixStep) / $g_iGAME_WIDTH)
	...
	If $g_iAndroidAdbMinitouchMode = 0 Then
		...
		;SetLog("TCPSend touch down (x,y)" & $x & "," & $y & "  wait: " & $sleep)
	Else
		AndroidAdbSendMinitouchShellCommand($send)
		;SetLog("ADBSend touch down (x,y)" & $x & "," & $y & "  wait: " & $sleep)
	EndIf
   ...
   ; touch move
	...
	For $i = 1 To $loops
		...
		If $g_iAndroidAdbMinitouchMode = 0 Then
			TCPSend($g_bAndroidAdbMinitouchSocket, $send)
			;SetLog("TCPSend touch move (x,y)" & int($x) & "," & int($y) & "  wait: " & $sleep)
		Else
			AndroidAdbSendMinitouchShellCommand($send)
			;SetLog("ADBSend touch move (x,y)" & int($x) & "," & int($y) & "  wait: " & $sleep)
		EndIf
	Next
   ...
   ; touch up
	...
	If $g_iAndroidAdbMinitouchMode = 0 Then
		TCPSend($g_bAndroidAdbMinitouchSocket, $send)
		;SetLog("TCPSend touch up")
	Else
		AndroidAdbSendMinitouchShellCommand($send)
		;SetLog("ADBSend touch up")
	EndIf
	_Sleep($botSleep)
...
Func CheckEmuNewVersions()
	...
	Switch $g_sAndroidEmulator
		...
		Case "Nox"
			$NewVersion = GetVersionNormalized("7.0.1.9")
		Case Else
