...
Func ClanGamesChallenges($sReturnArray, $makeIni = False, $sINIPath = "", $bDebug = False)
   ...
   Local $SpellChallenges[12][5] = [ _
	...
	Local $NightChallenges[31][5] = [ _
		...
		["GaintProblem",				"Gaint Problem",				 6,  1, 1], _
...
Func CollectClanGamesRewards()
	...
	; Collect from the exposed 5 reward columns
	For $i = 0 to 4
		Local $avRewards = SearchColumn($aiColumn)

		If Not IsArray($avRewards) Or UBound($avRewards, $UBOUND_ROWS) < 1 Then Return False
	...
	; drag to expose last column
	ClickDrag(755, 354, 755 - 47, 354, 200, "SLOW")

	Local $aiLastColumn[4] = [743, 190, 838, 494]

	Local $avRewards = SearchColumn($aiLastColumn)
	
	If Not IsArray($avRewards) Or UBound($avRewards, $UBOUND_ROWS) < 1 Then Return False
	
	DebugClanGamesRewards($avRewards, "Last")
	...
	Local $aCollectRewardsButton = findButton("ClanGamesCollectRewards", Default, 1, True)
...
Func SearchColumn($aiColumn)
	...
	Local $iMax = 30
	While 1
		$avRewards = findMultiple($sRewardsDir, $sSearchArea, $sSearchArea, 0, 1000, 0, "objectname,objectpoints", True)
