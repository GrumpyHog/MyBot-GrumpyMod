...
;Global $g_iMinDarkElixirUpgrade = 17000000
;Global $g_iMinElixirUpgrade = 290000
...
Func Laboratory($debug=False)
	...
	; check if current Elixir or Dark Elixir is less than the minimum upgrade 
	If $g_aiCurrentLoot[$eLootElixir] < $g_iMinimumLabUpgElixir or $g_aiCurrentLoot[$eLootDarkElixir] < $g_iMinimumLabUpgDarkElixir Then 
		SetLog("Resources below minimum for Lab upgrades!")
		Return
	EndIf

	;Click Laboratory
	;BuildingClickP($g_aiLaboratoryPos, "#0197")
	If Not LocateLaboratory() Then Return False
	...
	Local $iCurPage = 1
	Local $sCostResult
	Local $iMaxResourcesCost = 100000000
	Local $iMinElixirCost = $iMaxResourcesCost
	Local $iMinDarkElixirCost = $iMaxResourcesCost
	...
	Else ; users choice is any upgrade
			...
			If UBound($aPageUpgrades, 1) >= 1 Then ; if we found any troops
				For $i = 0 To UBound($aPageUpgrades, 1) - 1 ; Loop through found upgrades
					...
					If $sCostResult = "" Then ; not enough resources
						...
						Local $sCostResultRed = GetLabCostResultRed($aCoords)
						SetLog("Lab Upgrade " & $aTempTroopArray[0] & " requires " & $sCostResultRed)
						
						If IsDarkTroop($aTempTroopArray[0]) Or IsDarkSpell($aTempTroopArray[0]) Then
							
							If Number($sCostResultRed) < $iMinDarkElixirCost Then
								$iMinDarkElixirCost = Number($sCostResultRed)
								SetLog("Minimum Dark: " & $iMinDarkElixirCost)
							EndIf
						ElseIf IsElixirTroop($aTempTroopArray[0]) Or IsElixirSpell($aTempTroopArray[0]) Or IsSiegeMachine($aTempTroopArray[0]) Then
							If Number($sCostResultRed) < $iMinElixirCost Then
								$iMinElixirCost = Number($sCostResultRed)
								SetLog("Minimum Elixir: " & $iMinElixirCost)
							EndIf
						EndIf
		...
		If $iMinElixirCost = $iMaxResourcesCost And $iMinDarkElixirCost = $iMaxResourcesCost Then
			SetLog("There is nothing left to upgrade!")
			
			; disable Auto Lab Upgrade in GUI
			$g_bAutoLabUpgradeEnable = False
			GUICtrlSetState($g_hChkAutoLabUpgrades, $g_bAutoLabUpgradeEnable ? $GUI_CHECKED : $GUI_UNCHECKED)
			chkLab()
		Else
			SetLog("Minumum Elixir :" & $iMinElixirCost)
			SetLog("Minumum Dark Elixir :" & $iMinDarkElixirCost)
			$g_iMinimumLabUpgElixir = $iMinElixirCost
			$g_iMinimumLabUpgDarkElixir = $iMinDarkElixirCost
		EndIf
...
Func IsSiegeMachine($Troop)
	Local $iIndex = TroopIndexLookup($Troop, "IsSiegeMachine")
	;SetLog("Siege Machine Troop Index :" & $iIndex)
	If $iIndex >= $eWallW And $iIndex <= $eFlameF Then Return True
	Return False
EndFunc   ;==>IsSiegeMachine
...
Func LaboratoryUpgrade($name, $aCoords, $sCostResult, $debug = False)
	...
	Else
		...
		If isGemOpen(True) = False Then ; check for gem window
			...
			; reset minimum resources for upg
			$g_iMinimumLabUpgElixir = 0
			$g_iMinimumLabUpgDarkElixir = 0
...
Func GetLabCostResultRed($aCoords)
	Local $iCurSlotOnPage, $iCurSlotsToTheRight, $sCostResult
	$iCurSlotsToTheRight = Ceiling( ( Int($aCoords[0]) - Int(StringSplit($sLabTroopsSection, ",")[1]) ) / ($iSlotWidth + $iDistBetweenSlots) )
	If Int($aCoords[1]) < $iYMidPoint Then ; first row
		$iCurSlotOnPage = 2*$iCurSlotsToTheRight - 1
		$sCostResult = getLabUpgrdResourceRed( Int(StringSplit($sLabTroopsSection, ",")[1]) + 10 + ($iCurSlotsToTheRight-1)*($iSlotWidth + $iDistBetweenSlots),  Int(StringSplit($sLabTroopsSection, ",")[2]) + 76)
	Else; second row
		$iCurSlotOnPage = 2*$iCurSlotsToTheRight
		$sCostResult = getLabUpgrdResourceRed( Int(StringSplit($sLabTroopsSection, ",")[1]) + 10 + ($iCurSlotsToTheRight-1)*($iSlotWidth + $iDistBetweenSlots),  $iYMidPoint + 76)
	EndIf
	Return $sCostResult
EndFunc
...
Func LabNextPage($iCurPage, $iPages, $iYMidPoint)
	Local $iLPage = Random(300,340,1) ; last page
	Local $iNPage = 635 ; full page
	Local $iYPage = Random(420,520,1) 
	Local $iXPage = Random(650,700,1)

	If $iCurPage >= $iPages Then Return ; nothing left to scroll
	If $iCurPage = $iPages-1 Then ; last page
		;ClickDrag(700, $iYMidPoint, 360, $iYMidPoint)
		ClickDrag($iXPage, $iYPage, $iXPage - $iLPage, $iYPage)
	Else
		;ClickDrag(700, $iYMidPoint, 65, $iYMidPoint)
		ClickDrag($iXPage, $iYPage, $iXPage - $iNPage, $iYPage)
	EndIf
EndFunc
...
Func CloseLabWindow($iDelay = 500)
	If Not ClickB("CloseLabWindow") Then
		SetLog("Failed to find the CloseLabWindow button!", $COLOR_ERROR)
		ClickAway()
	EndIf
