...
Func chkShieldStatus($bChkShield = True, $bForceChkPBT = False)
	...
	; Normal 'shield' does not work in Legend
	If $g_aiCurrentLoot[$eLootTrophy] > 5000 Then Return

	; There is a 'grey area' where you are in Legend but your trophies are < 5000 
	; Legend Badge has rotating colours which makes checking pixel colour unreliable
	Local $aTitan[4] = [143, 106, 0x90681B, 10]
	
	SetLog("Enough trophies to be in Legend?")
	SetLog("Trophies : " & $g_aiCurrentLoot[$eLootTrophy])
	
	; Trophies higher than 4580 check if in Titan: 4900 - 320 = 4580, Min Legend Trophies = 4900, 8 x 3* = 320 
	If $g_aiCurrentLoot[$eLootTrophy] > 4580 Then
		SetLog("Yes, Is it a Titan shield?")
		SetLog("Badge: 0x" & _GetPixelColor(143, 106, True))
		If Not _CheckPixel($aTitan, True, Default, "chkShieldStatus") Then
			SetLog("Nope, hence must be in Legend")
			Return	
		EndIf
	EndIf
