...
Func RequestCC($bClickPAtEnd = True, $sText = "")
	...
	Local $sSearchDiamond = GetDiamondFromRect("718,580,840,614")
	...
	If _Sleep($DELAYREQUESTCC1) Then Return
	If $bClickPAtEnd Then CloseWindow("CloseTrain")
...
Func _makerequest($aRequestButtonPos)
	...
	; First Time Request CC copy saved request to local variable MOD
	If $g_bFirstTroopsRequest == True Then
		$sRequestTroopsText = $g_sRequestTroopsText
		SetLog("First time requesting troops")
	EndIf

	SetLog("Troops Request Text: " & $sRequestTroopsText)
	;
	...
	Else
		If $sRequestTroopsText <> "" Then
			...
			; First Troops request done
			$g_bFirstTroopsRequest = False
...
Func IsFullClanCastleType($CCType = 0) ; Troops = 0, Spells = 1, Siege Machine = 2
	...
	Else
		If _ColorCheck(_GetPixelColor($aCheckCCNotFull[$CCType], 470, True), Hex(0xDC363A, 6), 30) Then ; red symbol
			...
			If $aiRequestCountCC[$CCType] = 0 Or $aiRequestCountCC[$CCType] >= $g_iMaxCCTroopSpace - $CCType * 38 Then
				Return False
			Else
				Local $sCCReceived = getOcrAndCapture("coc-ms", 289 + $CCType * 183, 468, 60, 16, True, False, True) ; read CC (troops 0/$g_iMaxCCTroopSpace or spells 0/2)
...
Func CheckCCArmy()
	...
	For $i = 0 To 2
		If $g_aiClanCastleTroopWaitQty[$i] = 0 And $g_aiClanCastleTroopWaitType[$i] < $eTroopCount Then $g_aiCCTroopsExpected[$g_aiClanCastleTroopWaitType[$i]] = $g_iMaxCCTroopSpace ; expect troop type only. Do not care about qty
	Next
...
Func ShowExpectedCC()
	SetLog("Current Troop CC")
	For $i = 0 to $eTroopCount - 1
		If $g_aiCurrentCCTroops[$i] > 0 Then
			SetLog($g_asTroopNames[$i] & " x " & $g_aiCurrentCCTroops[$i])
		EndIf
	Next

	SetLog("Expected Troop CC")
	For $i = 0 to $eTroopCount - 1
		If $g_aiCCTroopsExpected[$i] > 0 Then
			SetLog($g_asTroopNames[$i] & " x " & $g_aiCCTroopsExpected[$i])
		EndIf
	Next

	SetLog("Current Spell CC")
	For $i = 0 to $eSpellCount - 1
		If $g_aiCurrentCCSpells[$i] > 0 Then
			SetLog($g_asSpellNames[$i] & " x " & $g_aiCurrentCCSpells[$i])
		EndIf
	Next

	SetLog("Expected Spell CC")
	For $i = 0 to $eSpellCount - 1
		If $g_aiCCSpellsExpected[$i] > 0 Then
			SetLog($g_asSpellNames[$i] & " x " & $g_aiCCSpellsExpected[$i])
		EndIf
	Next
	
		SetLog("Current Siege CC")
	For $i = 0 to $eSiegeMachineCount - 1
		If $g_aiCurrentCCSiegeMachines[$i] > 0 Then
			SetLog($g_asSiegeMachineNames[$i] & " x " & $g_aiCurrentCCSiegeMachines[$i])
		EndIf
	Next

	SetLog("Expected Siege CC")
	For $i = 0 to $eSiegeMachineCount - 1
		If $g_aiCCSiegeExpected[$i] > 0 Then
			SetLog($g_asSiegeMachineNames[$i] & " x " & $g_aiCCSiegeExpected[$i])
		EndIf
	Next
EndFunc