...
Func LocateLab($bCollect = True)
	...
	If Not LocateLaboratory() Then 
		If Not _LocateLab() Then Return False
	EndIf
...
Func _LocateLab($bCollect = True)
	...
	SetLog("Manual Lab Location", $COLOR_INFO)
...
Func LocateLaboratory()
	Local Static $aiLocateError[8]=[0, 0, 0, 0, 0, 0, 0, 0] 
	Local $sImgDir = @ScriptDir & "\imgxml\Buildings\Laboratory\"
	...
	If $g_aiLaboratoryPos[0] > 0 And $g_aiLaboratoryPos[1] > 0 Then
		...
		If $aResult[0] = 2 Then ; We found a valid building name
			...
			Else
				ClickAway()
				SetDebugLog("Stored Laboratory Position is not valid.", $COLOR_ERROR)
				SetDebugLog("Found instead: " & $aResult[1] & ", " & $aResult[2] & " !", $COLOR_DEBUG)
				SetDebugLog("Village position: " & $g_aiLaboratoryPos[0] & ", " & $g_aiLaboratoryPos[1], $COLOR_DEBUG, True)
				ConvertToVillagePos($g_aiLaboratoryPos[0],$g_aiLaboratoryPos[1])
				SetDebugLog("Real position: " & $g_aiLaboratoryPos[0] & ", " & $g_aiLaboratoryPos[1], $COLOR_DEBUG, True)
				;$g_aiLaboratoryPos[0] = -1
				;$g_aiLaboratoryPos[1] = -1
				$aiLocateError[$g_iCurAccount] += 1
				
				SetLog("Village[" & $g_iCurAccount & "] Laboratory Locate Error: " & $aiLocateError[$g_iCurAccount], $COLOR_DEBUG)
	...
	SetLog("Looking for Laboratory...", $COLOR_ACTION)
			...
			If StringInStr($aObjectpoints, "|") Then
				...
				For $j = 0 To UBound($tempObbjs) - 1
					...
					If UBound($tempObbj) = 2 Then
						$g_aiLaboratoryPos[0] = Number($tempObbj[0]) ;+ 9
						$g_aiLaboratoryPos[1] = Number($tempObbj[1]) ;+ 15
	...
	If $g_aiLaboratoryPos[0] > 0 And $g_aiLaboratoryPos[1] > 0 Then
		...
		If $aResult[0] = 2 Then ; We found a valid building name
			...
			Else
				ClickAway()
				SetDebugLog("Found Laboratory Position is not valid.", $COLOR_ERROR)
				SetDebugLog("Found instead: " & $aResult[1] & ", " & $aResult[2] & " !", $COLOR_DEBUG)
				SetDebugLog("Village position: " & $g_aiLaboratoryPos[0] & ", " & $g_aiLaboratoryPos[1], $COLOR_DEBUG, True)
				ConvertToVillagePos($g_aiLaboratoryPos[0],$g_aiLaboratoryPos[1])
				SetDebugLog("Real position: " & $g_aiLaboratoryPos[0] & ", " & $g_aiLaboratoryPos[1], $COLOR_DEBUG, True)
	...
	SetLog("Can not find Laboratory.", $COLOR_ERROR)
	Return False
EndFunc   ;==>LocateLaboratory()