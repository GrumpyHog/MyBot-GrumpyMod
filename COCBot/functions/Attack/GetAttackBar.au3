...
Func GetAttackBar($bRemaining = False, $pMatchMode = $DB, $bDebug = False)
	...
	If UBound($aAttackBar) = 0 Or Not $bRemaining Then
		...
		SetLog("GetAttackBar(): Finished Image Search in: " & StringFormat("%.2f", __TimerDiff($iAttackbarStart)) & " ms") ;Debug
	...
	For $i = 0 To UBound($aAttackBar, 1) - 1
		If $aAttackBar[$i][1] > 0 Then
			...
			If $bRemaining Then
				...
				SetLog("GetAttackBar(): Troop (x,y) " & $aAttackBar[$i][5] - 15 & "," & $aAttackBar[$i][6])
				...
				If _CheckPixel($aTroopIsDeployed, True) Then
					...
					If StringRegExp($aAttackBar[$i][0], $sKeepRemainTroops, 0) = 0 Then
						SetLog("GetAttackBar(): Troop " & $aAttackBar[$i][0] & " already deployed, now removed") ;
						ContinueLoop
					Else
						SetLog("GetAttackBar(): Troop " & $aAttackBar[$i][0] & " already deployed, but stays") ;
					EndIf
				EndIf
			Else
...
Func ExtendedAttackBarCheck($aAttackBarFirstSearch, $bRemaining, $sSearchDiamond)
	...
	Local $iLastTroop = _ArrayMaxIndex($aAttackBarFirstSearch, 0, -1, -1, 1)
	Local $sLastTroopName = $aAttackBarFirstSearch[$iLastTroop][0]
	;Local $sLastTroopName = $aAttackBarFirstSearch[_ArrayMaxIndex($aAttackBarFirstSearch, 0, -1, -1, 1)][0]
