						...
						If CheckCsvValues("MAKE", 2, $value2) Then
							...
							If CheckCsvValues("MAKE", 1, $value1) And CheckCsvValues("MAKE", 5, $value5) Then
								...
								If CheckCsvValues("MAKE", 8, $value8) Then ; Vector is targeted towards building v7.2
									...
									If $value3 >= 1 And	$value3 <= 5 Then ; check for valid number of drop points
										Local $tmpArray = _MakeTargetDropPoints(Eval($sidex), $value3, $value4, $value8)
										;_MakeTargetDropPoints(Eval($sidex), $value3, $value4, $value8)
										If @error Then
...
Func ParseAttackCSV_MainSide($debug = False)
	...
	SetLog("Func - ParseAttackCSV_MainSide")
