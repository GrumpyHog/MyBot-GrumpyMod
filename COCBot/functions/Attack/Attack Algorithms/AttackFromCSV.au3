...
Func FindWallCSV(ByRef $aCSVExternalWall, ByRef $aCSVInternalWall)
	...
	For $i = 0 To UBound($ExternalArea) - 1
			SetLog("ExternalArea : " & $i & ", 0:" & $ExternalArea[$i][0] & ", 1:" & $ExternalArea[$i][1] & ", 2:" & $ExternalArea[$i][2])
	Next

	; find the attacking side
	For $i = 0 To UBound($ExternalArea) - 1
		If $MAINSIDE = $ExternalArea[$i][2] Then
			SetLog("Found MAINSIDE")
			SetLog("ExternalArea : " & $i & ", 0:" & $ExternalArea[$i][0] & ", 1:" & $ExternalArea[$i][1] & ", 2:" & $ExternalArea[$i][2])
...
; Example .......: No
; ===============================================================================================================================
Func _MakeTargetDropPoints($side, $target, $addtiles, $building)
	;_MakeTargetDropPoints($side, $pointsQty, $addtiles, $building)
	...
	Local $aEdgeCoord[2] ;attack SIDE Coord
	...
	If IsArray($aBuildingLoc) Then
		...
		If UBound($aBuildingLoc, 1) > 1 And IsArray($aBuildingLoc[1]) Then ; cycle thru all building locations
			...
			For $p = 0 To UBound($aBuildingLoc) - 1
				...
				SetLog($g_sBldgNames[$BuildingEnum] & "(x,y) :" & $array[0] & "," & $array[1], $COLOR_INFO)

				#cs
				Local $x1 = $aEdgeCoord[0]
				Local $y1 = $aEdgeCoord[1]
				...
				$x = $array[0] - $aEdgeCoord[0]
				$y = $array[1] - $aEdgeCoord[1]
				$z = Sqrt($x * $x + $y * $y)

				ReDim $aBuildingDis[UBound($aBuildingDis) + 1][3]
				$aBuildingDis[UBound($aBuildingDis) - 1][0] = $z
				$aBuildingDis[UBound($aBuildingDis) - 1][1] = $array[0]
				$aBuildingDis[UBound($aBuildingDis) - 1][2] = $array[1]
			...
			_ArraySort($aBuildingDis, 0, 0, 0) ;Sort based on distance

			Local $iNumberOfTargets = UBound($aBuildingDis) -1
			
			For $p = 0 to $iNumberOfTargets
				SetLog("Building " & $p & " :Distance - " & $aBuildingDis[$p][0] & " (x,y): " & $aBuildingDis[$p][1] & "," & $aBuildingDis[$p][2])
			Next
			
			Local $iDis = Number($target) - 1 ; target 1 - x where 1 is closest x is farest
												; subtract 1 to index distance
			If $iDis > $iNumberOfTargets Then $iDis = $iNumberOfTargets
			
			$aLocation[0] = $aBuildingDis[$iDis][1]
			$aLocation[1] = $aBuildingDis[$iDis][2]
			
			SetLog("Building Target (x,y): " & $aLocation[0] & "," & $aLocation[1])
