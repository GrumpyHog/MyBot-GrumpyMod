...
Func GetTrainPos(Const $iIndex)
	...
	If $iIndex >= $eLSpell And $iIndex <= $eBtSpell Then
		Local $sFilter = String($g_asSpellShortNames[$iIndex - $eLSpell]) & "_*"
...
; given a filename and index, return [x coord, y coord, color pixel at xy, tolerance]
Func GetVariable(Const $ImageToUse, Const $iIndex)
	SetLog("TrainIT - Image Search for " & GetTroopName($iIndex))
	; Initial Timer
