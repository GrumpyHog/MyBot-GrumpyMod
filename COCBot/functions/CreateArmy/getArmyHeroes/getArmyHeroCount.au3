...
Func getArmyHeroCount($bOpenArmyWindow = False, $bCloseArmyWindow = False, $CheckWindow = True, $bSetLog = True)
	...
	If $bCloseArmyWindow Then CloseWindow("CloseTrain")
...
Func LabGuiDisplay() ; called from main loop to get an early status for indictors in bot bottom
	...
	If _DateIsValid($iLastTimeChecked[$g_iCurAccount]) Then
		...
		SetLog("Lab LastCheck: " & $iLastTimeChecked[$g_iCurAccount] & ", Check DateCalc: " & $iLastCheck) ; debug

		; do LabGuiDisplay every 6hrs if Lab running, every 1hrs if not
		If _DateIsValid($g_sLabUpgradeTime) Then
			Local $iLabTime = _DateDiff('n', _NowCalc(), $g_sLabUpgradeTime)
			SetLog("Lab LabUpgradeTime: " & $g_sLabUpgradeTime & ", Lab DateCalc: " & $iLabTime)
			
			; A check each 6 hours [6*60 = 360] or when Lab research time finishes ; debug
			If $iLabTime > 0 And $iLastCheck <= 360 Then Return
		Else
			; A check every hours
			If $iLastCheck <= 60 Then Return
		EndIf
	EndIf
