...
Func TrainCustomArmy()
	...
	debugTroopsSpells($g_aiArmyCustomTroops, $g_aiArmyCompSpells, 'Custom Train')
	...
	SetLog("Troop Queue : " & $bEmptyTroopQueue)
	SetLog("Spell Queue : " & $bEmptySpellQueue)
	...
	For $i = 0 To (UBound($rWTT) - 1)
		...
		If $rWTT[$i][1] > 0 Then ; If Count to Train Was Higher Than ZERO
			...
			If $rWTT[$i][1] > 0 Then
				...
				If IsSuperTroop($rWTT[$i][0]) Then
					If Not IsActiveSuperTroop($rWTT[$i][0]) Then
						$g_sBoostSuperTroop = $rWTT[$i][0]
						SetLog("Exiting Train Troop to Boost: " & $rWTT[$i][0], $COLOR_INFO)
						;ContinueLoop
						Return False
					EndIf
				EndIf
...
; 0xD3D3CB greyed out troop/spell colour - needs a rewrite!
...
Func RemoveExtraTroops($toRemove)
	...
	If UBound($toRemove) > 0 Then ; If needed to remove troops
	    ...
	    If Not _CheckPixel($aButtonRemoveTroopsOK2, True) Then; If no 'Okay' button found to verify that we accept the changes
			...
			; Click Away, Necessary! due to possible errors/changes
			CloseWindow("CloseTrain")
...
Func GetSlotRemoveBtnPosition($iSlot, $bSpells = False)
	...
	SetLog("iRemoveX :" & $iRemoveX)
	SetLog("iRemoveX :" & $iRemoveY)
...
Func GetSlotNumber($bSpells = False)
	Select
		Case $bSpells = False
		Local Const $Orders = [$eBarb, $eSBarb, $eArch, $eSArch, $eGiant, $eSGiant, $eGobl, $eSGobl, $eWall, $eSWall, $eBall, $eWiza, $eSWiza, $eHeal, $eDrag, $eSDrag, $eYeti, $ePekk, $eBabyD, $eInfernoD, $eMine, $eEDrag, $eDragR, _
...
Func TrainArmyNumber($abQuickTrainArmy)
	...
	If $g_bRandomQuickTrain Then
		RandomTrainArmyNumber($abQuickTrainArmy)
		Return
	EndIf
...
; $abQuickTrainArmy is 1D array [boolean, boolean, boolean]
Func RandomTrainArmyNumber($abQuickTrainArmy)
	Local $asSearchArea[3] = ["720,270,850,380", "720,380,850,490", "720,490,850,600"]

	Local $iArmyNumber
	Local $iHowManyArmy = 1
	Local $bArmySelect = False

	SetLog("Let's find which QT Army has be selected")
	; Find number of QT Army selected
	For $i = 0 to 2
		If $abQuickTrainArmy[$i] Then
			$iArmyNumber = $i
			$iHowManyArmy = $iHowManyArmy + 1
			SetLog("QT Army : " & $iArmyNumber +1 & " is selected")
		EndIf
	Next

	; More than 1 QT Army then Randomly select one
	If $iHowManyArmy > 1 Then
		SetLog("Multiple QT Armies selected, going to randomly choose one!")
		While 1
			$iArmyNumber = Random(0,2,1)
			if $abQuickTrainArmy[$iArmyNumber] Then

				$g_iQuickTrainArmyActive = $iArmyNumber

				SetLog("QT Army : " & $iArmyNumber + 1 & " selected for training")

				Local $sArea=$asSearchArea[$g_iQuickTrainArmyActive]
				...
				;debugTroopsSpells($g_aiArmyCompTroops, $g_aiArmyCompSpells)

				Local $aiTrainButton = decodeSingleCoord(findImage("QuickTrainButton", $g_sImgQuickTrain, GetDiamondFromRect($sArea), 1, True))

				If UBound($aiTrainButton, 1) = 2 Then
					ClickP($aiTrainButton)
					SetLog(" - Making the Army " & $g_iQuickTrainArmyActive + 1, $COLOR_INFO)

					If _Sleep(500) Then Return
				Else
					SetLog("Can't Army: " & $g_iQuickTrainArmyActive + 1)
				EndIf
...
Func IsActiveSuperTroop($sSuperTroopShortName)
	...
	SetLog("IsActiveSuperTroop :" & $sImageToUse[1])
	...
	Local $asResult = DllCallMyBot("FindTile", "handle", $g_hHBitmap2, "str", $sImageToUse[1], "str", "FV", "int", 1)
	...
	If IsArray($asResult) Then
		...
		If $asResult[0] = "0" Then
			SetLog("No " & $sSuperTroopShortName & " Icon found!", $COLOR_ERROR)
		ElseIf $asResult[0] = "-1" Then
			SetLog("TrainSystem.au3 IsActiveSuperTroop(): ImgLoc DLL Error Occured!", $COLOR_ERROR)
		ElseIf $asResult[0] = "-2" Then
			SetLog("TrainSystem.au3 IsActiveSuperTroop(): Wrong Resolution used for ImgLoc Search!", $COLOR_ERROR)
		Else
			If $g_bDebugSetlogTrain Then SetLog("String: " & $asResult[0])
				...
				If UBound($aResult) > 1 Then
					...
					If UBound($aCoordinates) > 1 Then
						...
						Return True
					Else
						SetLog("Don't know how to train the troop with index " & $sSuperTroopShortName & " yet.")
					EndIf
				Else
					SetLog("Don't know how to train the troop with index " & $sSuperTroopShortName & " yet..")
				EndIf
