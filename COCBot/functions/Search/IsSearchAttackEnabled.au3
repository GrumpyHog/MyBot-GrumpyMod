...
Func IsSearchAttackEnabled()
	...
	If Not $g_bLaunchAttack[$g_iCurAccount] And $g_AttackPlannerLegendLimit[$g_iCurAccount] Then
		If _DateIsValid($g_sNewLeagueTime[$g_iCurAccount]) Then
			$iWaitTime = _DateDiff('s', _NowCalc(), $g_sNewLeagueTime[$g_iCurAccount]) ; time from now until new league day 
			SetLog("NL Time: " & $g_sNewLeagueTime[$g_iCurAccount] & ", Time to NL: " & $iWaitTime/60 & " min")
