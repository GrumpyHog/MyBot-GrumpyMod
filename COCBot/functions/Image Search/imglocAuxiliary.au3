...
Func CloseWindow($sWindowName = "" , $iDelay = 500)
	Local $aiButton

	; search for CloseButton
	If $sWindowName = "CloseBoost" Or $sWindowName = "CloseDD" Or $sWindowName = "CloseTreasury" Then
		$aiButton = findButton($sWindowName, Default, 1, True)
	Else
		$aiButton = findButton($sWindowName, "CloseWindow*", 1, True)
	EndIf

	If IsArray($aiButton) And UBound($aiButton) >= 2 Then
		ClickP($aiButton, 1)
		SetLog("Window Closed : " & $sWindowName)
		If _Sleep($iDelay) Then Return
		Return True
	EndIf

	; cannot find CloseButton, clear screen using ClickAway
	SetLog("CloseWindow Failed : " & $sWindowName, $COLOR_ERROR)
	ClickAway()
	If _Sleep($iDelay) Then Return
...
Func GetButtonDiamond($sButtonName)
	...
	Switch $sButtonName
		...
		case "CloseTreasury"
			$btnDiamond = GetDiamondFromRect("665,160,715,205")
		case "CloseBoost"
			$btnDiamond = GetDiamondFromRect("675,165,730,215")
		case "CloseLab"
			$btnDiamond = GetDiamondFromRect("705,120,755,170")
		case "CloseDD"
			$btnDiamond = GetDiamondFromRect("680,165,735,220")
		case "CloseTrain"
