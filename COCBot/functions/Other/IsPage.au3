...
Func IsLaunchAttackPage()
	Local $bResult = False
	Local $i = 0
	Local $sImgDir = @ScriptDir & "\imgxml\Attack\Search\"
	Local $sSearchArea = GetDiamondFromRect("4,46,258,680") ; Tabs comprising of practise, single player and multi player

	$g_bLeagueAttack = False ; reset League flag

	While $i < 30
	
		Local $avPlayerTab = findMultiple($sImgDir, $sSearchArea, $sSearchArea, 0, 1000, 1, "objectname,objectpoints", True)

		If IsArray($avPlayerTab) And UBound($avPlayerTab, $UBOUND_ROWS) > 0 Then
			Local $avPlayerTabResult = $avPlayerTab[0] ; first item
			Local $sPlayerTabName = $avPlayerTabResult[0] ; get name
			Local $aPlayerTabCoord = StringSplit($avPlayerTabResult[1], ",", $STR_NOCOUNT) ; get coord
		
			; 2 types of MultiPlayer: Normal or Legend
			If StringInStr($sPlayerTabName, "MultiPlayer", 0) > 0 Then
				$bResult = True
				SetLog("Found Multiplayer Tab", $COLOR_INFO)
				ExitLoop
			EndIf
			
			If StringInStr($sPlayerTabName, "LegendLeague", 0) > 0 Then
				$bResult = True
				$g_bLeagueAttack = True
				SetLog("Found Legend League Tab", $COLOR_INFO)
				ExitLoop
			EndIf
			
			; Do we need to click tab?
			;ClickP($aCoordinates, 1)
			;SetLog("Click on MultiPlayer/LegendLeague Tab?", $COLOR_INFO)
			;If _Sleep(100) Then Return False	
		Else
			SetLog("Failed to find Launch Attack Page", $COLOR_ERROR)
			If $g_bDebugImageSave Then SaveDebugImage("IsLaunchAttackPage", False)
		EndIf
...
Func IsBattleMachineUpgradePage($bSetLog = True, $iLoop = 30)
	...
	If $bSetLog Then SetLog("Cannot find Battle Machine Upgrade Window...", $COLOR_ERROR) ; in case of $i = 29 in while loop
	If $g_bDebugImageSave Then SaveDebugImage("IsBattleMachineUpgradePage")
	If $iLoop > 1 Then AndroidPageError("IsBattleMachineUpgradePage")
	Return False
EndFunc   ;==>IsBattleMachineUpgradePage