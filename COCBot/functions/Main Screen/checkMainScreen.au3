...
Func _checkMainScreen($bSetLog = Default, $bBuilderBase = Default) ;Checks if in main screen
	...
	$i = 0
	$iErrorCount = 0
	$iCheckBeforeRestartAndroidCount = 10
...
;	ZoomOut()
;	Local $aChatTabPixel = decodeSingleCoord(findImage("ChatTabPixel", $g_sImgChatTabPixel, GetDiamondFromRect("1,450,50,310"), 1, True))
;	If UBound($aChatTabPixel) > 1 Then 
