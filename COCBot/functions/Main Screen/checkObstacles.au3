...
Func _checkObstacles($bBuilderBase = False, $bRecursive = False) ;Checks if something is in the way for mainscreen
	...
	If $g_sAndroidGameDistributor = "Magic" Then
		If ClashOfMagicAdvert() Then Return True
	EndIf
	...
	If UBound(decodeSingleCoord(FindImageInPlace("Maintenance", $g_sImgMaintenance, "270,70,640, 160", False))) > 1 Then ; Maintenance Break
		$Result = getOcrMaintenanceTime(300, 580, "Check Obstacles OCR Maintenance Break=")         ; OCR text to find wait time
	...
	If UBound(decodeSingleCoord(FindImageInPlace("Maintenance", $g_sImgCMaintenance, "270,70,640, 160", False))) > 1 Then ; Maintenance Break
		$Result = getOcrMaintenanceTime(206, 584, "Check Obstacles OCR Maintenance Break=")         ; OCR text to find wait time
		Local $iMaintenanceWaitTime = 0
		Local $avTime = StringRegExp($Result, "([\d]+)[Mm]|(soon)|([\d]+[Hh])", $STR_REGEXPARRAYMATCH)
		If UBound($avTime, 1) = 1 And Not @error Then
			If UBound($avTime, 1) = 3 Then
...
Func ConnectionLost($bRecursive, $bDebugLog=False)
	...
	SetLog("No Connection Lost Image found! (in " & Round(TimerDiff($hTimer) / 1000, 2) & " seconds)", $COLOR_DEBUG)
