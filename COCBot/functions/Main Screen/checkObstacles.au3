...
;	If PersonalBreak() Then Return True

;	If OutOfSync() Then Return True
	
;	If Inactive() Then Return True

;	If RateGame() Then Return True

;	If MiscMessage() Then Return True

;	If GameUpdate() Then checkObstacles_StopBot($msg) ; stop bot

;	If Banned() Then checkObstacles_StopBot($msg) ; stop bot
	...
	; Maintenance Simplified Chinese
	If UBound(decodeSingleCoord(FindImageInPlace("Maintenance", $g_sImgCMaintenance, "270,70,640, 160", False))) > 1 Then ; Maintenance Break
		$Result = getOcrChineseMaintenanceTime(206, 584)         ; OCR text to find wait time
		Local $iMaintenanceWaitTime = 0
		Local $avTime = StringRegExp($Result, "([\d]+)[Mm]|(soon)|([\d]+[Hh])", $STR_REGEXPARRAYMATCH)
		If UBound($avTime, 1) = 1 And Not @error Then
			If UBound($avTime, 1) = 3 Then
...
Func _checkObstacles1($bBuilderBase = False, $bRecursive = False) ;Checks if something is in the way for mainscreen
	Local $msg, $x, $y, $Result
	$g_bMinorObstacle = False
	...
	; Maintenance Simplified Chinese
	If UBound(decodeSingleCoord(FindImageInPlace("Maintenance", $g_sImgCMaintenance, "270,70,640, 160", False))) > 1 Then ; Maintenance Break
		$Result = getOcrCMaintenanceTime(206, 584, "Check Obstacles OCR Maintenance Break=")         ; OCR text to find wait time
		Local $iMaintenanceWaitTime = 0
		Local $avTime = StringRegExp($Result, "([\d]+)[Mm]|(soon)|([\d]+[Hh])", $STR_REGEXPARRAYMATCH)
		If UBound($avTime, 1) = 1 And Not @error Then
			If UBound($avTime, 1) = 3 Then
...
Func ConnectionLost($bRecursive)
	...
	Local $aiConnectionLost = decodeSingleCoord(findImage("ConnectLost", $sImgConnectionLost, GetDiamondFromRect("175,290,400,350"), 1, True))
	...
	If IsArray($aiConnectionLost) and UBound($aiConnectionLost, 1) = 2 Then
		...
		Local $aiDevice = decodeSingleCoord(findImage("device", $sImgDevice, GetDiamondFromRect("230,345,360,360"), 1, True))
	
		If IsArray($aiDevice) and UBound($aiDevice, 1) = 2 Then
			SaveDebugImage("AnotherDeviceConnected", False)
		
			SetLog("Another device conected")
			If $g_iAnotherDeviceWaitTime > 3600 Then
				SetLog("Another Device has connected, waiting " & Floor(Floor($g_iAnotherDeviceWaitTime / 60) / 60) & " hours " & Floor(Mod(Floor($g_iAnotherDeviceWaitTime / 60), 60)) & " minutes " & Floor(Mod($g_iAnotherDeviceWaitTime, 60)) & " seconds", $COLOR_ERROR)
				PushMsg("AnotherDevice3600")
			ElseIf $g_iAnotherDeviceWaitTime > 60 Then
			...
			If IsArray($aiReloadBtn) and UBound($aiReloadBtn, 1) = 2 Then
				...
				checkObstacles_ReloadCoC($aiReloadBtn, "#0127", $bRecursive)
			Else
			...
			Return True		
		Else
			SaveDebugImage("ConnectLost", False)
