   ...
   $g_bClanGamesCompleted = False
	...
	While 1
		...
		If Not $g_bIsClientSyncError And Not $g_bIsSearchLimit Then
			...
			; Halt mode if out of attacks
			If Not $g_bLaunchAttack[$g_iCurAccount] Then
				If _DateIsValid($g_sNewLeagueTime[$g_iCurAccount]) Then
					Local $iWaitTime = _DateDiff('n', _NowCalc(), $g_sNewLeagueTime[$g_iCurAccount]) ; time from now until new league day (minutes)
					SetLog("NL Time: " & $g_sNewLeagueTime[$g_iCurAccount] & ", Time to NL: " & $iWaitTime & " min")
	
					If $iWaitTime < 0 Then 
						$g_bLaunchAttack[$g_iCurAccount] = True
						SetLog("Legend Attacks can continue!")
			...
			;SetLog("$g_iAndroidClientHeight :" & $g_iAndroidClientHeight)
			...
			; Ensure, that wall upgrade is last of the home village upgrades
...
Func _Idle() ;Sequence that runs until Full Army
	...
	While $g_bIsFullArmywithHeroesAndSpells = False
		...
		;_RunFunction('BuilderBase') ; grumpy
...
Func AttackMain() ;Main control for attack functions
	If Not $g_bLaunchAttack[$g_iCurAccount] Then Return
	...
	If IsSearchAttackEnabled() Then
		If (IsSearchModeActive($DB) And checkCollectors(True, False)) Or IsSearchModeActive($LB) Then
			...
			If $g_bRestart Then Return
			If Not $g_bLaunchAttack[$g_iCurAccount] Then Return ; Problem encountered in PrepareSearch
			VillageSearch()
			If $g_bOutOfGold Then Return ; Check flag for enough gold to search
			If Not $g_bRunState Then Return
			If $g_bRestart Then Return
...
Func __RunFunction($action)
	...
	Switch $action
		...
		Case "BoostSuperTroop"
			SetLog("SuperTroop : " & $g_sBoostSuperTroop)
			;If $g_iBoostSuperTroopIndex <> -1 Then BoostSuperTroop($g_iBoostSuperTroopIndex)
			If $g_sBoostSuperTroop <> "" Then BoostSuperTroop($g_sBoostSuperTroop)
